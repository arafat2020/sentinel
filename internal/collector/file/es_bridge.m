...
static void handle_event(sentinel_es_client *state,
                         const es_message_t *message) {
  if (state == NULL || state->callback == NULL || message == NULL) {
    return;
  }
  ...
  case ES_EVENT_TYPE_NOTIFY_CREATE: {
    ...
    state->callback(ES_EVENT_TYPE_NOTIFY_CREATE, pid, ppid, path, NULL);
  ...
  case ES_EVENT_TYPE_NOTIFY_WRITE: {
    ...
    state->callback(ES_EVENT_TYPE_NOTIFY_WRITE, pid, ppid, path, NULL);
  ...
  case ES_EVENT_TYPE_NOTIFY_UNLINK: {
    ...
    state->callback(ES_EVENT_TYPE_NOTIFY_UNLINK, pid, ppid, path, NULL);
  ...
  case ES_EVENT_TYPE_NOTIFY_RENAME: {
    ...
    if (message->event.rename.destination_type ==
      ...
      destination = message->event.rename.destination.existing_file->path.data;
    }

    state->callback(ES_EVENT_TYPE_NOTIFY_RENAME, pid, ppid, destination,
                    source);
...
int sentinel_es_client_create(sentinel_es_client **out_client,
                              sentinel_es_event_callback callback) {
  if (out_client == NULL || callback == NULL) {
    return -1;
  }

  sentinel_es_client *state = calloc(1, sizeof(sentinel_es_client));
  ...
  es_new_client_result_t result = es_new_client(
      &state->client, ^(es_client_t *client, const es_message_t *message) {
        (void)client;
...
int sentinel_es_client_subscribe(sentinel_es_client *state) {
  ...
  es_return_t result =
      es_subscribe(state->client, events, sizeof(events) / sizeof(events[0]));
